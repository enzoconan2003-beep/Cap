...
mod tests {
    ...
    fn matches_by_owner_name() {
        let exclusion = owner_exclusion("Reel");
        assert!(exclusion.matches(None, Some("Reel"), None));
    ...
    fn matches_owner_and_title_requires_both() {
        let exclusion = WindowExclusion {
            bundle_identifier: None,
            owner_name: Some("Reel".to_string()),
            window_title: Some("Reel Camera".to_string()),
        };
        assert!(exclusion.matches(None, Some("Reel"), Some("Reel Camera")));
        assert!(!exclusion.matches(None, Some("Reel"), Some("Wrong Title")));
    ...
    fn instant_mode_removes_camera_exclusion() {
        let exclusions = vec![
            ...
            title_exclusion("Reel Camera"),
            title_exclusion("Reel Settings"),
            title_exclusion("Reel Recording Controls"),
        ];

        let filtered = filter_for_instant_mode(exclusions, "Reel Camera");

        assert_eq!(filtered.len(), 3);
        assert!(
            filtered
                .iter()
                .all(|e| e.window_title.as_deref() != Some("Reel Camera"))
        );
        assert!(
            filtered
                .iter()
                .any(|e| e.window_title.as_deref() == Some("Reel"))
        );
        assert!(
            filtered
                .iter()
                .any(|e| e.window_title.as_deref() == Some("Reel Settings"))
        );
        assert!(
            filtered
                .iter()
                .any(|e| e.window_title.as_deref() == Some("Reel Recording Controls"))
        );
    }

    #[test]
    fn instant_mode_noop_when_camera_absent() {
        let exclusions = vec![title_exclusion("Reel"), title_exclusion("Reel Settings")];
    ...
    fn instant_mode_handles_empty_list() {
        let filtered = filter_for_instant_mode(vec![], "Reel Camera");
    ...
    fn default_exclusions_contain_camera() {
        ...
        assert!(
            ...
            "Default exclusions must include 'Reel Camera' — instant mode filtering depends on this"
        );
    }
    ...
    fn default_exclusions_only_match_camera_by_title() {
        ...
        let camera = defaults
            ...
            .expect("Reel Camera must be in default exclusions");
